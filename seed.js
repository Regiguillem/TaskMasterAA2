...
const Panel = require('../TaskMaster-Pr2/app/models/Panel');
const Task = require('../TaskMaster-Pr2/app/models/Task');
...
async function initData() {
    try {
      ...
      for (let i = 0; i < savedPanels.length; i++) {
        savedPanels[i].tasks = tasks.filter(task => task.panel.equals(savedPanels[i]._id));
